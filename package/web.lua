...
<aside class="panel"><h2>连接状态</h2><div class="state"><div class="metric"><span>设备 → 主机</span><strong><i class="dot" id="dot"></i><b id="online">未连接</b></strong></div><div class="metric"><span>事件流</span><strong><code id="url">-</code></strong></div><div class="metric"><span>屏幕字体</span><strong id="font">CLAWD CONSOLE · RGB SUBPIXEL</strong></div><div class="metric"><span>最近信息</span><strong id="detail">-</strong></div></div></aside></section></main>
<script>const API="]=] .. api_prefix .. [=[";const $=id=>document.getElementById(id);function path(v){v=(v||"").trim();return !v?"/events":v[0]==="/"?v:"/"+v}function url(){return "http://"+$('host').value.trim()+":"+$('port').value.trim()+path($('path').value)}function msg(t,c){$('message').textContent=t;$('message').className="status "+(c||"")}function paint(d){$('host').value=d.host||"";$('port').value=d.port||17321;$('path').value=d.path||"/events";$('url').textContent=d.url||url();$('online').textContent=d.online?"已连接":"未连接";$('dot').className="dot "+(d.online?"on":"");let f=d.font||{};$('font').textContent=(f.family||"MONTSERRAT")+" · "+(f.rendering||"GRAYSCALE");$('detail').textContent=f.error||d.detail||"等待连接"}
